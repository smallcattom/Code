...
bool solve(int x){
    int l = a[x] , r = 0;
    ans.resize(0);ans.PB(a[x]);
    for (int i = 1 ; i < m ; ++i){
        int res = 0;
        if(i & 1){
            while(res < SZ(a) && (l >= r + a[res] || a[res] == ans.back())) ++res;
            if (res >= SZ(a)) return 0;
            r += a[res];
            ans.PB(a[res]);
        }
        else{
            while(res < SZ(a) && (l + a[res] <= r || a[res] == ans.back())) ++res;
            if (res >= SZ(a)) return 0;
            l += a[res];
            ans.PB(a[res]);
        }
...
int main(){
    ...
    for (int i = 0 ; i < A.length() ; ++i)
        if (A[i] == '1') a.PB(i + 1);
    for(int i = 0 ; i < SZ(a) ; ++i) if (solve(i)){
            puts("YES");
        for (int j = 0 ; j < m ; ++j){
            if (j) printf(" ");
            printf("%d" , ans[j]);
